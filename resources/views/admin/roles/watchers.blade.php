<x-app-layout>
    ...
    <div x-data="{
            ...
            menu: null,
            editId: null,
            form: { watcher_id: '', target_id: '', scope: 'participant' },
            openCreate() {
                this.editId = null;
                this.form = { watcher_id: '', target_id: '', scope: 'participant' };
                this.modal = true;
            },
            openEdit(rule) {
                this.editId = rule.id;
                this.form = { watcher_id: rule.watcher_id, target_id: rule.target_id, scope: rule.scope };
                this.menu = null;
                this.modal = true;
            }
         }" class="max-w-4xl">
        ...
        <div class="space-y-3">
            @forelse($rules as $i => $rule)
                <div class="bg-white rounded-xl border border-gray-200 px-5 py-4 flex items-center gap-3">
                    ...
                    <span class="text-sm text-gray-500 truncate">{{ \App\Models\DocumentWatcher::SCOPES[$rule->scope] ?? $rule->scope }}</span>
                    ...
                    <div class="flex items-center gap-2 ml-auto shrink-0">
                        ...
                        <div class="relative">
                            ...
                            <div x-show="menu === {{ $i }}" x-cloak @click.outside="menu = null"
                                ...
                                <button type="button"
                                        @click="openEdit(@js(['id' => $rule->id, 'watcher_id' => $rule->watcher_id, 'target_id' => $rule->target_id, 'scope' => $rule->scope]))"
                                        class="w-full text-left px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Изменить</button>
                                <form method="POST" action="{{ route('admin.roles.watchers.destroy', $rule) }}"
                                      onsubmit="return confirm('Удалить правило наблюдения?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-red-50">Удалить</button>
                                </form>
                            </div>
                        </div>
                    </div>
        ...
        {{-- Модалка создания / редактирования правила --}}
        <div x-show="modal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <form method="POST"
                  :action="editId ? '{{ url('admin/roles/watchers') }}/' + editId : '{{ route('admin.roles.watchers.store') }}'"
                ...
                <h2 class="text-lg font-bold text-gray-900 mb-5" x-text="editId ? 'Изменить правило' : 'Правило наблюдения'"></h2>

                <div class="space-y-4">
                    <div>
                        <label class="text-xs font-semibold text-gray-600 uppercase tracking-widest block mb-1.5">Наблюдатель</label>
                        <select name="watcher_id" x-model="form.watcher_id" required
                    ...
                    <div>
                        ...
                        <select name="target_id" x-model="form.target_id" required
                                class="w-full text-sm border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#5B4FE8]">
                            <option value="" disabled>— выберите —</option>
                            @foreach($users as $u)
                                <option value="{{ $u->id }}">{{ $u->name }}</option>
                    ...
                    <div>
                        ...
                        <select name="scope" x-model="form.scope"
                                class="w-full text-sm border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#5B4FE8]">
                            @foreach(\App\Models\DocumentWatcher::SCOPES as $value => $label)
                ...
                <div class="flex items-center gap-3 mt-6">
                    <button type="submit" class="px-6 py-2.5 bg-[#5B4FE8] text-white rounded-lg text-sm font-medium hover:bg-indigo-700"
                            x-text="editId ? 'Сохранить' : 'Создать правило'"></button>
