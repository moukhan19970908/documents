<x-app-layout>
    <x-slot name="title">На согласование — Командировки</x-slot>

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">На согласование</h1>
            <p class="text-sm text-gray-500 mt-1">Командировки, ожидающие вашего решения</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden" x-data="{ rejectId: null, revisionId: null, comment: '' }">
                    ...
                    @forelse($trips as $trip)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-5 py-3.5">
                                <div class="font-medium text-gray-900">{{ $trip->user->name }}</div>
                            ...
                            <td class="px-5 py-3.5 text-xs text-gray-500">
                                {{ $trip->current_step }} / {{ $trip->route?->steps->count() ?? '?' }}
                            </td>
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-1.5 justify-end">
                                    ...
                                    <form action="{{ route('trips.approve', $trip) }}" method="POST">
                                        @csrf
                                    ...
                                    <button type="button" @click="rejectId = {{ $trip->id }}; comment = ''"
                                    ...
                                    <button type="button" @click="revisionId = {{ $trip->id }}; comment = ''"
        ...
        <div x-show="rejectId !== null" x-transition
            ...
            <div class="bg-white rounded-xl w-full max-w-md p-6" @click.outside="rejectId = null">
                ...
                <form :action="`/trips/${rejectId}/reject`" method="POST">
                    @csrf
        ...
        <div x-show="revisionId !== null" x-transition
            ...
            <div class="bg-white rounded-xl w-full max-w-md p-6" @click.outside="revisionId = null">
                ...
                <form :action="`/trips/${revisionId}/revision`" method="POST">
                    @csrf
                    <textarea name="comment" x-model="comment" rows="3" required
                    ...
                    <div class="flex gap-2 justify-end">
                        <button type="button" @click="revisionId = null"
                                class="px-4 py-2 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50">Отмена</button>
                        <button type="submit"
                                class="px-4 py-2 text-sm bg-orange-500 text-white rounded-lg hover:bg-orange-600">На доработку</button>
                    </div>
                </form>
            </div>
        </div>
